...
const leases = computed(() => {
    ...
    return all.filter((l) => {
        if (onlyStatic.value && l.dynamic) return false;
        if (!q) return true;
...
async function makeStatic(l) {
    if (!window.confirm([
        `จอง ${l.address} ให้เครื่องนี้ถาวร?`,
        '',
        `MAC: ${l.macAddress}`,
    ...
    try {
        ...
        toast.success(`จอง ${l.address} ถาวรแล้ว`);
        await load();
    } catch (err) {
...
async function removeLease(l) {
    ...
    const msg = l.dynamic
        ...
        : [`ลบการจองถาวรของ ${l.address}?`, '', 'IP นี้จะกลับเข้ากองให้เครื่องอื่นใช้ได้',
...
<template>
    <div class="panel">
        ...
        <template v-if="data">
            <div class="chips">
                <span class="chip">ทั้งหมด <b class="v2-num">{{ data.summary.total }}</b></span>
                <span class="chip ok">ใช้งานอยู่ <b class="v2-num">{{ data.summary.bound }}</b></span>
                <span class="chip">จองถาวร <b class="v2-num">{{ data.summary.static }}</b></span>
                <span class="chip">แจกอัตโนมัติ <b class="v2-num">{{ data.summary.dynamic }}</b></span>
                <span v-for="s in data.servers" :key="s.name" class="chip srv">
                    {{ s.name }} · {{ s.interface }} · pool {{ s.addressPool }} · lease {{ s.leaseTime }}
                </span>
            </div>

            <div class="bar">
                <input v-model="search" class="v2-input" placeholder="ค้นหา IP, MAC, ชื่อเครื่อง, หมายเหตุ...">
                <label class="chk">
                    <input v-model="onlyStatic" type="checkbox"> เฉพาะที่จองถาวร
                </label>
                <span class="count v2-num">{{ sorted.length }} รายการ</span>
            </div>
            ...
            <div class="tablewrap">
                <table>
                    <thead>
                        <tr>
                            <th>IP Address</th><th>MAC Address</th><th>ชื่อเครื่อง</th>
                            <th>ประเภท</th><th>สถานะ</th><th>เห็นล่าสุด</th><th class="right">จัดการ</th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr v-for="l in sorted" :key="l.id">
                            ...
                            <td>
                                <span class="badge" :class="l.dynamic ? 'b-dyn' : 'b-static'">
                                    {{ l.dynamic ? 'แจกอัตโนมัติ' : 'จองถาวร' }}
                                </span>
                            </td>
                            <td>
                            ...
                            <td class="right">
                                <div class="rowbtns">
                                    <button
                                        ...
                                        title="ให้เครื่องนี้ได้ IP เดิมทุกครั้ง"
                                        :disabled="busy === l.id" @click="makeStatic(l)"
                                    >
                                        <i class="fa-solid" :class="busy === l.id ? 'fa-spinner fa-spin' : 'fa-thumbtack'"></i> จองถาวร
                                    </button>
                                    <button
...
.bar .v2-input { flex: 1; min-width: 200px; }
.chk { display: flex; align-items: center; gap: 7px; font-size: .82rem; cursor: pointer; white-space: nowrap; }
.count { font-size: .8rem; color: var(--v2-text-muted); font-weight: 600; margin-left: auto; }
