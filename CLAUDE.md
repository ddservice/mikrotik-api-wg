...
- **2026-09-07** — **v2 ผ่านการคลิกด้วยมือจริงครบทุกหน้าแล้ว** — ครั้งแรกที่ทำได้ใน session
  ...
  - Chrome extension เชื่อมต่อและ**ต่อ local server ได้** รอบนี้ (ก่อนหน้านี้เชื่อมเบราว์เซอร์
    ได้แต่ต่อ localhost ไม่ได้) รันผ่าน `UI_DEFAULT=v2` + `scripts/fake-routeros.js` และ
    ล็อกอิน `admin` เข้าคลิกจริง
  - คลิกครบ ทุกหน้า **ไม่มี console error เลย**: Login, Overview (9 การ์ด + กราฟ default
    pppoe-out1), Hotspot (5 แท็บ + แท็บ archive มี dropdown กรองสาขา), PPPoE (การ์ดรอบบิล
    ซ่อนอยู่ตอนปิด — gate ทำงาน), Multi-WAN (กด Read → กล่อง "ขั้นต่อไป" ขึ้นพร้อมปุ่ม —
    ปิดปัญหา "กด read แล้วไงต่อ"), Firewall (16 บริการ + group pills กรองถูก),
    Log (กรองสาขา + chip scope + 4 แท็บ), สิทธิ์เมนู (8 เมนู + ล็อก 2 + นับสด),
    Settings 6 แท็บ (รอบบิล switch ปิดทุกสาขา default, Storage 4 vitals cards)
  - **ยืนยันสองการแก้หน้าตรวจสุขภาพในเบราว์เซอร์จริง**: พอร์ต ether3 ที่ไม่ได้ใช้ไม่ขึ้น
    เตือน (เหลือ ether4 ที่เคยใช้แล้วหลุด), และ log link-down/PPPoE ครั้งเดียวไม่ขึ้นเป็น
    เรื่องควรดูอีก
  - **เรื่องที่พบ ไม่ใช่บั๊ก แต่ควรจำ:**
    - สวิตช์เปิดรอบบิลใช้ `window.confirm()` ซึ่ง**บล็อก browser automation** จนกดต่อไม่ได้
      (ผู้ใช้จริงกดได้ปกติ) — เป็น safety ที่ตั้งใจไว้ ไม่ใช่จุดบกพร่อง แต่ทำให้ automation
      ทดสอบ flow เปิดสวิตช์ไม่ได้ ต้องยืนยันผ่าน HTTP แทน
    - หน้า Firewall โชว์ error "no such command (/ip/firewall/address-list/print)" กับ
      เราท์เตอร์**จำลอง**ที่ไม่มี endpoint นั้น — บนเราท์เตอร์จริงมี และหน้าแสดง error สวย
      ไม่ crash · fixture ควรเพิ่ม endpoint นี้ถ้าจะเทสต์ Firewall ให้ครบในอนาคต
    - หน้า Storage ในโหมด JSON โหลด ~21 วินาที เพราะนับ 57k แถวจากไฟล์ตรง ๆ ระหว่างนั้น
      ขึ้น "กำลังตรวจ..." เฉย ๆ — **บน production (Supabase) เร็ว** เพราะ `head:true` count
      จึงเป็นเรื่องเฉพาะโหมด dev ไม่ต้องแก้
  - กฎข้อ 4 (ห้ามลบ `public/app.js`/`index.html` จนกว่าจะคลิกผ่าน v2 ด้วยมือ) — เงื่อนไข
    "คลิกผ่าน" ทำได้แล้ว แต่**ยังไม่ลบ v1** เพราะ (ก) ยังไม่ได้ deploy โค้ดล่าสุดขึ้น
    production (ข) การเลิก v1 เป็นการตัดสินใจของเจ้าของ ไม่ใช่ผลพลอยได้ของ session ทดสอบ
    การสลับ default ยังทำผ่าน `UI_DEFAULT` ได้เหมือนเดิม
  - ทั้งหมดนี้เป็นการตรวจสอบ ไม่มีการแก้โค้ด — จึงไม่มีอะไรต้อง deploy จากรายการนี้เอง

- **2026-09-06 (11)** — Health-check เตือนเหตุการณ์ปกติที่หายเองไปแล้ว — finding ที่เหลือ
  ในภาพเดียวกับพอร์ตที่ไม่ได้ใช้ (2026-09-06 (10))
  - "สายที่พอร์ตหลุด" และ "สาย PPPoE หลุด" ในรายงานหน้างาน **เกิดครั้งเดียวและเป็นของเก่า**
    (link-down เมื่อวาน, PPPoE เมื่อ 7 ชม.ก่อน) ทั้งคู่ต่อกลับเองแล้ว — finding ของ PPPoE
    เองยังเขียนว่า "ปกติจะต่อกลับเองภายในไม่กี่วินาที" · log เป็น buffer แบบวน เหตุการณ์
    ปกติของเมื่อวานยังค้างอยู่ ตรรกะเดิมเห็นแล้วเตือน "ควรดู" ทันทีโดยไม่สนความถี่หรืออายุ
    ชุดเดียวกับพอร์ตที่ไม่ได้ใช้: **การเตือนสิ่งที่ไม่ต้องทำอะไร ฝังเรื่องที่ต้องทำจริงไว้ในกอง**
  - เพิ่มแนวคิด "เหตุการณ์ที่หายเองได้" (`transient`) ให้กฎ log — สาย/PPPoE หลุดแล้วต่อกลับ
    แสดงสามระดับตามความถี่แทนที่จะเตือนทุกครั้ง:
    - เกิดไม่กี่ครั้ง (< `noteAt`) → **info** ไม่ขึ้นในรายการเตือนและไม่เด้ง Telegram
    - เกิดถี่ (≥ `noteAt`: link 4, PPPoE 6) → **warning** เริ่ม flapping ควรดู
    - เกิดถี่มาก (≥ `escalateAt`: PPPoE 20) → **critical**
  - ISP หลายรายตัด-ต่อ PPPoE วันละครั้งเป็นเรื่องปกติ การเตือนทุกครั้งคือ noise ที่ทำให้
    คนเลิกอ่านหน้านี้ · เหตุการณ์ที่ไม่ใช่ transient (รีบูต, DHCP แปลกปลอม, เดารหัส) ไม่เปลี่ยน
    — รีบูตครั้งเดียวอาจเป็นไฟดับ ควรรู้
  - ยืนยันผ่าน HTTP กับ fixture (มี link-down 1 ครั้ง): finding "สายที่พอร์ตหลุด" หายไป
    เหลือแต่ของจริง (DHCP แปลกปลอม + รีบูต + ether4 ที่เคยใช้แล้วหลุด) · `needsAttention`
    ไม่เด้งจากเหตุการณ์ปกติครั้งเดียวอีก (ไม่งั้น Telegram จะส่งเรื่องที่ไม่ต้องทำอะไร)
    3 เทสต์ใหม่ (**545 เทสต์**)

- **2026-09-06 (10)** — Health-check เตือนพอร์ตที่ไม่ได้ใช้ว่าเป็นปัญหา — รายงานจากหน้างาน
  - อาการ: หน้าตรวจสุขภาพขึ้น "มีพอร์ตที่ไม่มีสัญญาณ: ether3, ether4" ทั้งที่สองพอร์ตนั้น
    **ไม่ได้เสียบสายไว้ตั้งแต่แรก** ไม่ใช่ปัญหา
  - สาเหตุ: `router-health.js` เตือนทุกพอร์ต ether ที่ `running=false` และไม่ได้ `disabled`
    — แต่พอร์ตที่ไม่ได้ใช้ก็เข้าเงื่อนไขนี้พอดี เพราะไม่มีใครไป disable มัน
    **การรายงานสิ่งที่ไม่ต้องทำอะไร คือการฝังเรื่องที่ต้องทำจริงไว้ในกอง** — บทเรียนเดียว
    กับที่ health monitor ส่งเฉพาะเรื่องร้ายแรงเข้า Telegram (2026-09-05 (9))
  - แก้: แยก "พอร์ตที่ตั้งใจใช้แต่สายหลุด" ออกจาก "พอร์ตที่ไม่เคยใช้" ด้วยหลักฐานจาก
    `/interface/print` เอง — เตือนเฉพาะพอร์ตที่มีอย่างน้อยหนึ่งอย่าง: มี `comment`
    (แอดมินกำกับว่าใช้), เคยมีลิงก์ (`last-link-up-time` มีค่า = เคยมีสายเสียบจริง),
    หรือเคยมี traffic (`rx/tx-byte > 0`) · พอร์ตที่ไม่เข้าเลย = ไม่เคยใช้ ข้ามเงียบ ๆ
    เปลี่ยนหัวข้อเป็น "มีพอร์ตที่เคยใช้งานแต่ตอนนี้สายหลุด" ให้ตรงกับสิ่งที่ตรวจจริง
  - ยืนยันผ่าน HTTP กับ fixture (เพิ่ม ether3 = ไม่เคยใช้, ether4 = เคยใช้+comment แล้วหลุด):
    finding ขึ้นเฉพาะ **ether4** ส่วน ether3 หายไป · เทสต์คุมทั้งสองทาง — พอร์ตที่มี
    comment/last-link-up-time/traffic ต้องขึ้น, พอร์ตเปล่าและพอร์ตที่ปิดเองต้องไม่ขึ้น,
    และเราท์เตอร์ที่ทุกพอร์ตว่างต้องไม่เกิดการเตือนพอร์ตเลย 2 เทสต์ใหม่ (**542 เทสต์**)
  - อีกสองเรื่องในภาพ (สายที่พอร์ตหลุด, PPPoE หลุด) มาจาก log จริงของเราท์เตอร์และ
    **วิเคราะห์ถูกต้อง** — เป็นเหตุการณ์ที่เกิดขึ้นจริงพร้อม timestamp ไม่ได้แก้

- **2026-09-06 (9)** — เพิ่ม guard กันความลับหลุดเข้า git — สิ่งที่ควรมีตั้งแต่ครั้งแรก
  ที่ commit ความลับ (2026-07-30) และครั้งที่สอง (คีย์ R2, 2026-09-06)
  - repo นี้เพิ่ม guard ที่ตรวจ invariant ของทั้งไฟล์หลังเจอปัญหาทุกครั้ง
    (`validate-html`, `check-db-parity`, `check-routes`, `verify-backup`) แต่ **ความ
    ผิดพลาดที่เกิดซ้ำที่สุดและเสียหายมากที่สุด — ความลับหลุด — กลับไม่เคยมีตัวกัน**
  - `scripts/check-secrets.js` สแกนเฉพาะไฟล์ที่ `git ls-files` ติดตามอยู่ เพราะนั่นคือ
    สิ่งเดียวที่หลุดขึ้น GitHub ได้ ไฟล์ที่ gitignore ไว้ (`ecosystem.config.js`,
    `db/*.json`, `slips/`) มีความลับจริงได้ตามปกติและต้องไม่ทำให้ guard fail
    รูปแบบที่ตรวจคือของที่เคยหลุดจริงหรือหลุดแล้วเสียหายหนัก: คีย์ R2 สองตัวที่เคยหลุด
    (ตรง ๆ), JWT/Supabase service key, AWS/R2 secret, PEM private key, Slack/OpenAI token,
    LINE token ที่ยาวผิดปกติในซอร์ส · **placeholder** (`YOUR_`, `EXAMPLE`, `xxxx`) ถูก
    ยกเว้น ไม่งั้นไฟล์ template จะเตือนผิดตลอด
  - **เจอของจริงทันทีที่รันครั้งแรก**: JWT ค้างใน `scripts/vps-harden-ports-and-pm2.sh`
    ตรวจแล้วเป็นค่าตัวอย่าง `eyJ...YOUR_SERVICE_ROLE_KEY` ไม่ใช่คีย์จริง จึงเพิ่มการ
    ยกเว้น placeholder แทนที่จะปิดตาไฟล์นั้นทั้งไฟล์
  - **บั๊กที่เทสต์จับได้ก่อนใช้จริง**: เวอร์ชันแรกผูก ROOT ไว้กับที่ตั้งไฟล์ตายตัว จึง
    สแกน repo จริง (สะอาด) เสมอไม่ว่ารันจากที่ไหน — เทสต์ (ที่รันในไดเรกทอรี git ชั่วคราว)
    จับได้ว่า guard ผ่านทั้งที่ควร fail แก้เป็นยึด `git rev-parse` ของ `process.cwd()`
    ระหว่างแก้เจอปมที่สอง: บน Windows ที่ cwd เป็น UNC path (ไดรฟ์ที่ map ไว้) การ spawn
    git โดยไม่ระบุ cwd ทำให้ cmd.exe chdir ไม่ได้และ git พังหมด — ทุกคำสั่ง git จึงส่ง
    cwd เข้าไปตรง ๆ
  - เทสต์พิสูจน์ทั้งสองทางตามกติกาของ repo (guard ที่ไม่เคยถูกพิสูจน์ว่า fail ไม่ใช่
    guard): fail กับ JWT/PEM/คีย์ R2 จริง, ผ่านกับ placeholder และชื่อตัวแปรเฉย ๆ,
    และ**ไฟล์ที่มีคีย์จริงแต่ไม่ได้ track ต้องไม่ทำให้ fail** (เหมือน ecosystem.config.js
    บน VPS) · wire เข้า `npm run check` แล้ว 7 เทสต์ใหม่ (**541 เทสต์**)
  - ไม่ได้แก้ต้นเหตุที่หลุดไปแล้ว — ตัวนี้กันครั้งหน้า การหมุนคีย์ + ปิด repo (ดู
    2026-09-06 (8)) ยังเป็นสิ่งที่ต้องทำและเป็นสิทธิ์เจ้าของ
