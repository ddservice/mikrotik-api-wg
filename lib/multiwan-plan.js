...
/** resolver ต้นทางที่เราท์เตอร์จะใช้ เมื่อเปิดให้มันเป็น DNS ให้ลูกข่าย */
...
function buildFailoverPlan(analysis, opts = {}) {
    // ลงทับของที่มีอยู่แล้วจะได้ route/NAT/netwatch/scheduler ซ้อนกันเป็นชุด ๆ
    //
    // เจอตอนทดสอบ: กด apply ซ้ำไม่กี่ครั้งได้ route 30 เส้น netwatch 5 ตัว
    // และที่แย่กว่านั้นคือค่า orig= ที่ฝังไว้รอบหลังจะเป็นค่าที่ถูกดันไปแล้ว
    // ทำให้ถอนคืนไม่กลับค่าเดิมอีกต่อไป
    if (analysis && analysis.activeWan && analysis.activeWan.installed) {
        const e = new Error(
            'สาขานี้ติดตั้ง Multi-WAN failover ไว้อยู่แล้ว — ต้องกด Remove ก่อน ' +
            'จึงจะติดตั้งใหม่ได้' + String.fromCharCode(10) +
            'การลงทับจะได้ route และ NAT ซ้อนกัน และทำให้ถอนคืนค่าเดิมไม่ได้อีก'
        );
        e.alreadyInstalled = true;
        throw e;
    }
    ...
    // ---- ขั้นที่ 7 (ตัวเลือก): ทำให้ DNS ทนต่อการสลับ line ----
    //
    // ปิดไว้เป็นค่าเริ่มต้นเพราะขั้นนี้เปลี่ยนสิ่งที่ลูกข่ายได้รับจริง ต่างจากขั้นอื่น
    // ที่แตะแค่ routing table ผลจะเกิดเมื่อลูกข่ายต่อ DHCP ใหม่ ไม่ใช่ทันที
    //
    // ปัญหาที่แก้: ลูกข่ายได้ DNS ของ ISP สายหลักมาทาง DHCP พอสายหลักตาย
    // เส้นทางสลับสำเร็จแต่เปิดเว็บไม่ได้ เพราะ resolver นั้นเข้าถึงได้เฉพาะสายที่ตายไปแล้ว
    if (opts.dnsResilience && analysis.dns) {
        const d = analysis.dns;
        const upstream = (opts.dnsServers && opts.dnsServers.length
            ? opts.dnsServers : DEFAULT_DNS_SERVERS).join(',');

        // ---- ฝากคำสั่งคืนค่า DNS ไว้บนเราท์เตอร์ก่อน ----
        //
        // ปุ่ม Remove ถอนของที่ติดคอมเมนต์กำกับได้ แต่ค่า DNS กับ dns-server ของ
        // DHCP เป็น "การตั้งค่า" ไม่ใช่ "รายการ" จึงติดคอมเมนต์ไม่ได้และถอนไม่ได้
        // ด้วยวิธีเดียวกัน — ถ้าไม่ฝากไว้ กด Remove แล้วเราท์เตอร์จะยังเป็น resolver
        // และ DHCP ยังแจก IP ของเราท์เตอร์อยู่ ซึ่งคือ "ถอนแล้วไม่กลับ" แบบเดียว
        // กับบั๊ก default-route-distance ที่เจอมาแล้ว
        //
        // เก็บเป็นสคริปต์บนเราท์เตอร์ เพราะค่าเดิมต้องอยู่รอดแม้ฐานข้อมูลเราหาย
        const restore = [
            `/ip dns set servers="${d.upstreamServers.join(',')}" ` +
            `allow-remote-requests=${d.routerIsResolver ? 'yes' : 'no'}`
        ];
        (d.networks || []).forEach((n) => {
            if (!n.id) return;
            restore.push(`/ip dhcp-server network set [find where .id="${n.id}"] ` +
                         `dns-server="${n.dnsServer || ''}"`);
        });
        steps.push({
            id: 'dns-save-original',
            title: 'บันทึกค่า DNS เดิมไว้บนเราท์เตอร์ (สำหรับตอนถอน)',
            why: 'ค่า DNS เป็นการตั้งค่า ไม่ใช่รายการที่ติดคอมเมนต์ได้ ถ้าไม่เก็บค่าเดิมไว้ ' +
                 'ตอนกด Remove จะถอนไม่กลับ — เก็บบนเราท์เตอร์เพื่อให้ใช้ได้แม้ระบบเราไม่อยู่',
            risk: 'low',
            apply: {
                cmd: '/system/script/add',
                args: {
                    name: `${TAG}-dnsrestore`,
                    source: restore.join(NL),
                    comment: `${TAG} dns restore`
                }
            },
            undo: { type: 'remove-added', cmd: '/system/script/remove' }
        });

        // ---- กฎ firewall ข้างล่างอ้าง in-interface-list=WAN ต้องแน่ใจว่ามันมีจริง ----
        //
        // ถ้า list ไม่มีอยู่ กฎจะถูกสร้างสำเร็จแต่ไม่ match อะไรเลย = เราท์เตอร์
        // เป็น open resolver ทั้งที่หน้าจอบอกว่าปิดกั้นแล้ว ซึ่งอันตรายกว่าไม่ทำ
        const wl = analysis.wanList || { exists: false, missingMembers: [] };
        if (!d.routerIsResolver && !wl.exists) {
            steps.push({
                id: 'wan-list',
                title: 'สร้าง interface list ชื่อ WAN',
                why: 'กฎปิดกั้น DNS อ้าง in-interface-list=WAN ถ้า list ไม่มีอยู่ กฎจะถูกสร้าง ' +
                     'สำเร็จแต่ไม่ match อะไรเลย เราท์เตอร์จะเป็น open resolver ' +
                     'ทั้งที่หน้าจอบอกว่าปิดกั้นแล้ว',
                risk: 'low',
                apply: { cmd: '/interface/list/add', args: { name: 'WAN', comment: `${TAG} wan list` } },
                undo: { type: 'remove-added', cmd: '/interface/list/remove' }
            });
        }
        if (!d.routerIsResolver) {
            (wl.missingMembers || []).forEach((n) => {
                steps.push({
                    id: `wan-list-member-${n}`,
                    title: `เพิ่ม ${n} เข้า interface list WAN`,
                    why: 'ต้องเป็นสมาชิกของ list ถึงจะถูกกฎปิดกั้น DNS ครอบคลุม',
                    risk: 'low',
                    apply: {
                        cmd: '/interface/list/member/add',
                        args: { list: 'WAN', interface: n, comment: `${TAG} wan member` }
                    },
                    undo: { type: 'remove-added', cmd: '/interface/list/member/remove' }
                });
            });
        }
        ...
        if (!d.routerIsResolver) {
            steps.push({
                id: 'dns-resolver',
                title: `เปิดให้เราท์เตอร์เป็น DNS resolver (upstream: ${upstream})`,
                why: 'ตัวเราท์เตอร์ออก internet ทาง line ไหนก็ได้ตามที่ failover จัดให้ ' +
                     'ลูกข่ายที่ถาม DNS ผ่านเราท์เตอร์จึงใช้งานได้ต่อเนื่องแม้ line หลักตาย',
                risk: 'low',
                apply: {
                    cmd: '/ip/dns/set',
                    args: { servers: upstream, 'allow-remote-requests': 'yes' }
                },
                undo: {
                    cmd: '/ip/dns/set',
                    args: {
                        servers: d.upstreamServers.join(',') || '',
                        'allow-remote-requests': d.routerIsResolver ? 'yes' : 'no'
                    }
                }
            });

            // เปิด allow-remote-requests โดยไม่ปิดกั้นฝั่ง WAN = open DNS resolver
            // ซึ่งถูกใช้ขยายกำลังโจมตี DDoS ได้ ต้องมาคู่กันเสมอ ไม่ใช่ตัวเลือก
            steps.push({
                id: 'dns-block-wan',
                title: 'ปิดกั้น DNS จากฝั่ง WAN (กันเป็น open resolver)',
                why: 'การเปิด allow-remote-requests โดยไม่ปิดกั้นขาเข้าจาก WAN ทำให้เราท์เตอร์ ' +
                     'กลายเป็น open DNS resolver ซึ่งถูกใช้ขยายกำลังโจมตี DDoS ใส่คนอื่นได้ ' +
                ...
                apply: {
                    ...
                    args: {
                        ...
                        // place-before=0 บนรายการที่ยังว่างอยู่จะ error
                        // ("no such item") — ใส่เฉพาะเมื่อมีกฎอยู่แล้วจริง
                        ...(analysis.filterRuleCount > 0 ? { 'place-before': '0' } : {}),
                        comment: `${TAG} block dns udp from wan`
                    }
                },
                undo: { type: 'remove-added', cmd: '/ip/firewall/filter/remove' }
            });
            steps.push({
                id: 'dns-block-wan-tcp',
                title: 'ปิดกั้น DNS over TCP จากฝั่ง WAN',
                why: 'DNS ใช้ได้ทั้ง UDP และ TCP ปิดแค่ UDP อย่างเดียวยังเปิดช่องอยู่',
                risk: 'low',
                apply: {
                    cmd: '/ip/firewall/filter/add',
                    args: {
                        chain: 'input', protocol: 'tcp', 'dst-port': '53',
                        'in-interface-list': 'WAN', action: 'drop',
                        ...(analysis.filterRuleCount > 0 ? { 'place-before': '0' } : {}),
                        comment: `${TAG} block dns tcp from wan`
                    }
                },
                undo: { type: 'remove-added', cmd: '/ip/firewall/filter/remove' }
            });
        ...
        // ให้ DHCP แจก IP ของเราท์เตอร์เองเป็น DNS
        (d.networks || []).forEach((n) => {
            if (!n.id || !n.gateway) return;
            const handed = String(n.dnsServer || '').split(',').map((x) => x.trim()).filter(Boolean);
            if (handed.includes(n.gateway)) return;   // ตั้งถูกอยู่แล้ว
            steps.push({
                id: `dns-dhcp-${n.id}`,
                title: `DHCP network ${n.address || n.id}: แจก DNS = ${n.gateway}`,
                why: 'ชี้ลูกข่ายมาถาม DNS ที่เราท์เตอร์แทน resolver ของ ISP — ' +
                     'มีผลเมื่อลูกข่ายต่อ DHCP ใหม่ ไม่ใช่ทันที เครื่องที่ออนไลน์อยู่ไม่สะดุด',
                risk: 'medium',
                apply: {
                    cmd: '/ip/dhcp-server/network/set',
                    args: { '.id': n.id, 'dns-server': n.gateway }
                },
                undo: {
                    cmd: '/ip/dhcp-server/network/set',
                    args: { '.id': n.id, 'dns-server': n.dnsServer || '' }
                }
            });
        });
    }
...
function buildRollbackScript(plan) {
    ...
    const lines = [
        ...
        guard(`/ip firewall filter remove [find comment~"${TAG}"]`),
        guard(`/interface list member remove [find comment~"${TAG}"]`),
        guard(`/interface list remove [find comment~"${TAG}"]`),
        // คืนค่า DNS ก่อนลบสคริปต์ที่เก็บค่าเดิมไว้
        guard(`/system script run [find comment~"${TAG} dns restore"]`),
        guard(`/system script remove [find comment~"${TAG}"]`)
    ];
    plan.wans.forEach((w) => {
        if (!w.id) return;
