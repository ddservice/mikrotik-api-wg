...
/**
 * ดึง log ของวันเดียวออกมาทั้งหมด (วนหน้าไปเรื่อย ๆ จนครบ)
 * ตั้ง limit ไว้ 5000 ต่อหน้า — ใหญ่พอให้เร็ว แต่ไม่ใหญ่จนกิน memory
 */
async function fetchDay(db, type, dateStr) {
    ...
    let page = 1;
    for (;;) {
        const res = await def.fetch(db, { from: dateStr, to: dateStr, page, limit: 5000 });
        const batch = (res && res.logs) || [];
        rows.push(...batch);
        const pages = (res && res.pages) || 1;
        if (page >= pages || !batch.length) break;
        page++;
    }
    return rows;
}
