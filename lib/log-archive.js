 ...
 *
 * วิธีเดิมแปลงเป็นเวลาไทยด้วย toLocaleString แล้วให้ new Date() ตีความสตริงนั้น
 * ตามเขตเวลาของเครื่องอีกที จากนั้น toISOString() แปลงกลับเป็น UTC — สุทธิแล้ว
 * เลื่อน -7 ชม. จึงได้ "วันที่ตาม UTC" ไม่ใช่วันที่ตามเวลาไทย และผิดไปหนึ่งวัน
 * ทุกครั้งที่เวลาไทยยังไม่ถึง 07:00 น.
 *
 * ผลจริงที่เจอ 2026-08-30: งานปิดวันตอน 02:00 น. คำนวณ "เมื่อวาน" ได้เป็นวันที่ 28
 * ทั้งที่ควรเป็นวันที่ 29 ทำให้ไฟล์ปิดผนึกช้ากว่าที่ควรหนึ่งวันตลอด และตัวกันไม่ให้
 * ปิดวันที่ยังไม่จบ (dateStr >= bangkokToday()) ก็บล็อกการปิดวันที่ 29 ไปด้วย
 *
 * en-CA ให้รูปแบบ YYYY-MM-DD อยู่แล้ว จึงไม่ต้องประกอบสตริงเอง
 */
