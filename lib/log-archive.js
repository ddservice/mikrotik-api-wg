...
async function buildArchive(db, type, dateStr, { createdBy = 'System Auto' } = {}) {
    ...
    // DNS เก็บเป็นไฟล์ JSONL อยู่แล้ว — บีบอัดไบต์ตรง ๆ ไม่ต้อง parse แล้ว stringify กลับ
    // วันที่มีสามแสนกว่าแถว ถ้าแปลงเป็น object ทั้งหมดใช้ heap ~295 MB ซึ่งเกือบชน
    // ลิมิต 500M ของ PM2 ตอนงานกลางคืนทำงาน (วัดจริงบน VPS 2026-08-31)
    let gz = null;
    let recordCount = 0;

    if (type === 'dns') {
        const packed = dnsStore.gzipLiveDay(dateStr);
        if (packed) {
            gz = packed.buffer;
            recordCount = packed.rows;
        }
    }
    ...
    if (!gz) {
        ...
        gz = zlib.gzipSync(Buffer.from(jsonl, 'utf8'), { level: 9 });
    ...
    console.log(`[LogArchive] ${fileName} — ${recordCount} รายการ, ${gz.length} bytes, sha256 ${sha256.slice(0, 12)}…`);
    return record;
}
