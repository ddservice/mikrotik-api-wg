...
const RULES = [
    ...
    },
    {
        ...
        // เหตุการณ์ที่หายเองได้: เกิดครั้งเดียวแล้วต่อกลับ = ปกติ ไม่ต้องเตือน
        // เกิดซ้ำ ๆ = สายหลวม/อุปกรณ์มีปัญหา ค่อยยกเป็นเรื่องควรดู
        transient: true, noteAt: 4
    },
    {
        ...
        // จังหวะ ISP รีเซ็ตสายรายวัน หรือ RADIUS lag อาจเกิด 1-2 ครั้งแล้วต่อติด
        // เกิดไม่กี่ครั้ง = info, เกิด 3-7 ครั้ง = warning, เกิด 8+ ครั้ง = critical
        transient: true, noteAt: 3, escalateAt: 8
    },
    {
        code: 'pppoe-down',
        ...
        // ISP หลายรายตัดต่อ PPPoE วันละครั้งเป็นเรื่องปกติ เตือนทุกครั้งคือ noise
        // หลุดถี่ (flapping) ต่างหากที่เป็นปัญหา
        transient: true, noteAt: 6, escalateAt: 20
    },
    {
        code: 'reboot',
...
function summarize(rows) {
    ...
    // ปรับระดับตามความถี่ที่เกิด — ข้อความใน log เหมือนกันทุกตัวอักษร แต่ "เกิดครั้งเดียว
    // แล้วหายเอง" กับ "เกิดรัว ๆ" เป็นคนละเรื่องกันโดยสิ้นเชิง
    groups.forEach((g) => {
        const rule = RULES.find((r) => r.code === g.code);
        if (!rule) return;

        // เหตุการณ์ที่หายเองได้ (สาย/PPPoE หลุดแล้วต่อกลับ): เกิดไม่กี่ครั้ง = ปกติ
        // ลดเป็น info เพื่อไม่ให้ขึ้นในรายการเตือนและไม่เด้งเข้า Telegram
        // — เตือนเหตุการณ์ปกติที่หายไปแล้ว คือการฝังเรื่องจริงไว้ในกองจนไม่มีใครอ่าน
        if (rule.transient && rule.noteAt && g.count < rule.noteAt &&
            g.severity === SEVERITY.WARNING) {
            g.severity = SEVERITY.INFO;
            g.demoted = true;
            if (rule.code === 'pppoe-auth-failed') {
                g.meaning = g.meaning + ` (เกิด ${g.count} ครั้ง — อาจเป็นจังหวะ ISP รีเซ็ตสายรายวัน หากต่อติดแล้วยังไม่ถือว่าผิดปกติ)`;
            } else {
                g.meaning = g.meaning + ` (เกิด ${g.count} ครั้ง และต่อกลับเองแล้ว — ยังไม่ถือว่าผิดปกติ)`;
            }
        }

        // เกิดถี่เกินเกณฑ์ = ยกระดับเป็นเรื่องร้ายแรง
        if (rule.escalateAt && g.count >= rule.escalateAt && g.severity === SEVERITY.WARNING) {
