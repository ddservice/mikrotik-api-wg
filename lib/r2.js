...
/**
 * ไล่รายการอ็อบเจกต์ทั้งหมดใต้ prefix ที่กำหนด (ListObjectsV2)
 *
 * S3 ส่งกลับได้สูงสุด 1000 รายการต่อครั้ง แล้วให้ continuation token มา
 * ต้องวนจนกว่า IsTruncated จะเป็น false ไม่งั้นได้ข้อมูลไม่ครบแบบเงียบ ๆ
 * (บั๊กเดียวกับที่เคยทำให้ไฟล์ปิดผนึกขาดที่ 1000 แถวเมื่อ 2026-08-28)
 *
 * อ่าน XML ด้วย regex เพราะไม่อยากเพิ่ม dependency แค่เพื่อ parse
 * โครงสร้าง ListObjectsV2 คงที่และเรียบง่ายพอที่จะทำแบบนี้ได้อย่างปลอดภัย
 */
async function listObjects(prefix = '') {
    ...
    do {
        ...
        const xml = await request({ method: 'GET', query });
        ...
        keys.forEach((k, i) => out.push({ key: k, lastModified: dates[i] || null, size: sizes[i] || 0 }));
...
module.exports = { isConfigured, putObject, getObject, listObjects, request, cfg };
