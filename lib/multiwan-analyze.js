...
/**
 * สรุปกฎ mangle แต่ละข้อให้อ่านรู้เรื่อง — ไม่ใช่แค่บอกว่า "มี 6 rules"
 *
 * คนที่ต้องตัดสินใจต้องเห็นว่ากฎแต่ละข้อทำอะไรกับ traffic ก้อนไหน ถึงจะบอกได้ว่า
 * ทับกับของใหม่ตรงไหน การบอกแค่จำนวนเท่ากับโยนงานตรวจกลับไปให้คนทำเองทั้งหมด
 */
function describeMangle(rules) {
    return toArr(rules).map((m) => {
        const marks = m['new-routing-mark'] || m['new-connection-mark'] || '';
        const scope = [
            m['src-address'] ? 'src ' + m['src-address'] : '',
            m['dst-address'] ? 'dst ' + m['dst-address'] : '',
            m['in-interface'] ? 'เข้าทาง ' + m['in-interface'] : '',
            m['in-interface-list'] ? 'เข้าทางกลุ่ม ' + m['in-interface-list'] : '',
            m.protocol ? 'protocol ' + m.protocol : '',
            m['dst-port'] ? 'พอร์ต ' + m['dst-port'] : '',
            m['connection-mark'] && m['connection-mark'] !== 'no-mark' ? 'conn-mark ' + m['connection-mark'] : ''
        ].filter(Boolean).join(', ');
        ...
        return {
            ...
            mark: marks,
            scope: scope || 'ทุก traffic ที่ผ่าน chain นี้',
            comment: m.comment || '',
            // กฎที่ mark routing คือกฎที่ "แย่ง" การตัดสินใจเส้นทางไปจาก default route
            stealsRouting: m.action === 'mark-routing' || !!m['per-connection-classifier']
        };
    });
}

/** สรุป default route ให้เห็น distance ของแต่ละสาย ว่าใครมาก่อนมาหลัง */
function describeDefaultRoutes(routes) {
    return toArr(routes).map((r) => ({
        id: r['.id'] || '',
        gateway: r.gateway || '',
        ...
        routingTable: r['routing-table'] || r['routing-mark'] || 'main',
        comment: r.comment || ''
...
function analyzeState(state, opts = {}) {
    ...
    // mark-routing / PCC ที่มีอยู่แล้ว = ห้ามลง failover ทับ ไม่ใช่แค่ "ควรตรวจ"
    //
    // traffic ที่ถูกติด routing-mark จะไปใช้ routing table ของ mark นั้น ไม่ใช่ตาราง main
    // ที่ failover ของเราเขียน default route ลงไป ผลคือติดตั้งครบทุกขั้น รายงานสำเร็จ
    // แต่ traffic ของลูกค้าไม่ได้เดินตามนั้นเลย และตอนสายหลักตาย traffic ที่ถูก mark
    // ก็ยังวิ่งไปสายที่ตายไปแล้วอยู่ดี
    //
    // ที่อันตรายกว่านั้นคือขั้นตรวจสอบหลังติดตั้งจะ "ผ่าน" ด้วย เพราะ ping ที่ยิงยืนยัน
    // ออกจากตัวเราท์เตอร์เอง ซึ่งไม่ได้โดนกฎ mangle ชุดนี้ — เป็นความล้มเหลวแบบ
    // "รายงานว่าสำเร็จทั้งที่ไม่ได้ทำอะไร" ซึ่งแย่กว่าติดตั้งไม่สำเร็จไปตรง ๆ
    if (mangleConflicts.length > 0) {
        blockers.push({
            code: 'existing-mark-routing',
            message: `เราท์เตอร์ตัวนี้มีกฎ mangle ที่ mark routing / PCC อยู่แล้ว ${mangleConflicts.length} rules ` +
                     'ซึ่งจะดึง traffic ไปใช้ routing table ของตัวเอง ข้าม default route ที่ failover จะสร้าง — ' +
                     'ติดตั้งทับจะขึ้นว่าสำเร็จแต่ไม่มีผลจริงกับ traffic ของลูกค้า ' +
                     'ต้องปิดหรือลบกฎชุดนี้ก่อน แล้วค่อยติดตั้ง failover'
        });
    }
    ...
    if (defaults.length > 1) {
        warnings.push({
            ...
            message: `มี active default route อยู่ ${defaults.length} routes — ` +
                     ...
                     '(ดูข้อเสนอ distance ด้านล่าง — ระบบเรียงตามความเร็วที่กรอกไว้ให้แล้ว)'
        });
    }

    // ข้อเสนอว่าสายไหนควรเป็น primary — เรียงตามความเร็วที่กรอกไว้ (ไม่กรอก = เรียงตามลำดับที่เจอ)
    // เป็นข้อเสนอให้คนอ่านแล้วตัดสิน ไม่ใช่สิ่งที่ระบบจะไปตั้งเองโดยไม่ถาม
    const ranked = usable.slice().sort((a, b) => (b.speedMbps || 0) - (a.speedMbps || 0));
    const distancePlan = ranked.map((w, i) => ({
        interface: w.interface,
        kind: w.kind,
        speedMbps: w.speedMbps,
        proposedDistance: i + 1,
        role: i === 0 ? 'primary' : 'backup ลำดับ ' + i,
        reason: w.speedMbps
            ? `ความเร็วที่กรอกไว้ ${w.speedMbps} Mbps`
            : 'ยังไม่ได้กรอกความเร็ว — เรียงตามลำดับที่เจอบนเราท์เตอร์'
    }));
    ...
    return {
        ...
        defaultRouteDetail: describeDefaultRoutes(defaults),
        distancePlan,
        mangleConflicts,
        mangleDetail: describeMangle(mangleConflicts),
        blockers,
        warnings,
