...
const T = {
    ...
    diskCritical: 10, diskWarning: 20,    // % ที่เหลือ (สำหรับดิสก์ขนาดปกติ > 32MB)
    smallDiskCriticalKb: 800,             // KB ที่เหลือ (สำหรับ Flash ขนาดเล็ก <= 32MB)
    smallDiskWarningKb: 1600,            // KB ที่เหลือ (สำหรับ Flash ขนาดเล็ก <= 32MB)
...
function analyzeHealth(state) {
    ...
    if (totalHdd > 0) {
        const totalMb = totalHdd / 1048576;
        const freeKb = Math.round(freeHdd / 1024);
        const totalKb = Math.round(totalHdd / 1024);

        if (totalMb <= 32) {
            // เราท์เตอร์ Flash 16MB/เล็ก (เช่น hAP ac^2, hEX, hAP mini)
            // พื้นที่ปกติมักเหลือ ~1.2 - 1.8MB (10% คือ 1.5MB) จึงไม่ควรเตือน critical ทันที
            if (freeKb < T.smallDiskCriticalKb) {
                add('critical', 'พื้นที่เก็บข้อมูลบนเราท์เตอร์เหลือน้อยมาก',
                    `เหลือ ${freeKb} KB จาก ${totalKb} KB`,
                    'เราท์เตอร์อาจบันทึก log/backup ไม่ได้และเสี่ยงค้าง — ลบไฟล์เก่าในเมนู Files โดยเฉพาะไฟล์สำรองและ package ที่ไม่ใช้แล้ว');
            } else if (freeKb < T.smallDiskWarningKb) {
                add('warning', 'พื้นที่เก็บข้อมูลบนเราท์เตอร์เริ่มน้อย',
                    `เหลือ ${freeKb} KB จาก ${totalKb} KB`,
                    'รุ่น Flash 16MB พื้นที่จำกัด — แนะนำตรวจสอบและลบไฟล์เก่าในเมนู Files ก่อนเต็ม');
            }
        } else {
            // เราท์เตอร์ดิสก์ขนาดปกติ (> 32MB เช่น hEX S, RB3011, RB4011, x86, CHR)
            const freePct = Math.round((freeHdd / totalHdd) * 100);
            if (freePct < T.diskCritical) {
                add('critical', 'พื้นที่เก็บข้อมูลบนเราท์เตอร์ใกล้เต็ม',
                    `เหลือ ${freePct}% (${Math.round(freeHdd / 1048576)} MB จาก ${Math.round(totalMb)} MB)`,
                    'ลบไฟล์เก่าในเมนู Files โดยเฉพาะไฟล์สำรองและ log ที่ไม่ใช้แล้ว');
            } else if (freePct < T.diskWarning) {
                add('warning', 'พื้นที่เก็บข้อมูลบนเราท์เตอร์เริ่มน้อย',
                    `เหลือ ${freePct}% (${Math.round(freeHdd / 1048576)} MB จาก ${Math.round(totalMb)} MB)`,
                    'เฝ้าดูและพิจารณาลบไฟล์สำรองเก่าที่ไม่ได้ใช้แล้ว');
            }
        }
    }
    ...
    // ---- พอร์ตที่ "เคยใช้งาน" แต่ตอนนี้ลิงก์หลุด ----
    //
    // ของเดิมเตือนทุกพอร์ต ether ที่ running=false ซึ่งรวมพอร์ตที่ไม่ได้เสียบสายไว้
    // ตั้งแต่แรกด้วย (ไม่มีใครไป disable มัน) จึงขึ้นเตือน ether3/ether4 ที่ไม่ได้ใช้
    // ทั้งที่ไม่ใช่ปัญหา — รายงานสิ่งที่ไม่ต้องทำอะไร คือการฝังเรื่องที่ต้องทำจริงไว้ในกอง
    //
    // แยก "พอร์ตที่ตั้งใจใช้แต่สายหลุด" ออกจาก "พอร์ตที่ไม่เคยใช้" ด้วยหลักฐานว่ามันเคย
    // ทำงานจริง ซึ่ง /interface/print ให้มาได้เอง — ต้องมีอย่างน้อยหนึ่งอย่าง:
    //   - มี comment (แอดมินตั้งใจกำกับว่าใช้พอร์ตนี้)
    //   - เคยมีลิงก์ขึ้นมาก่อน (last-link-up-time มีค่า = เคยมีสายเสียบจริง)
    //   - เคยมี traffic ผ่าน (rx/tx-byte > 0)
    // พอร์ตที่ไม่เข้าเงื่อนไขเลย = ไม่เคยใช้ ข้ามไปเงียบ ๆ ไม่ใช่ปัญหา
    const everLinked = (i) => {
        const t = String(i['last-link-up-time'] || '').trim().toLowerCase();
        return t && !t.startsWith('jan/01/1970') && t !== '0';
    };
    const hadTraffic = (i) =>
        (parseInt(i['rx-byte'], 10) || 0) > 0 || (parseInt(i['tx-byte'], 10) || 0) > 0;
    const looksInUse = (i) =>
        String(i.comment || '').trim() !== '' || everLinked(i) || hadTraffic(i);
    ...
    const downPorts = (s.ifaces || []).filter((i) =>
        ...
        looksInUse(i));
    if (downPorts.length) {
        add('warning', 'มีพอร์ตที่เคยใช้งานแต่ตอนนี้สายหลุด', downPorts.map((i) => i.name).join(', '),
            'พอร์ตพวกนี้เคยมีอุปกรณ์ต่ออยู่ แต่ตอนนี้ไม่มีสัญญาณ — ตรวจสายและปลายทาง ' +
            '(พอร์ตที่ไม่ได้ใช้และไม่เคยเสียบสายจะไม่ขึ้นเตือน)');
    }
