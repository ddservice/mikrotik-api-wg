...
const WATCHED_DIRS = [
    ...
    { key: 'dns-logs', label: 'ประวัติเข้าเว็บ (วันนี้)', dir: path.join(ROOT, 'dns-logs'), growing: true },
...
async function buildReport(db) {
    const parts = await Promise.all([
        ...
        Promise.resolve(dnsStore.stats()).catch(() => null),
    ...
    const dnsFiles = parts[4];
    const dnsSites = parts[5];
    ...
    // ไม่นับตารางที่หยุดเขียนแล้ว — แถวใน 24 ชม.ล่าสุดของมันเป็นของก่อนย้ายระบบ
    // ถ้านับด้วยจะได้อัตราโตที่ไม่มีอยู่จริงและเตือน "จะเต็ม" ทุกวัน
    const growthBytesPerDay = (dbStats.tables || []).reduce((sum, t) => {
        if (t.legacy) return sum;
    ...
    return {
        ...
        // ที่เก็บ DNS แบบไฟล์ — นับแยกจาก Postgres เพราะเป็นคนละที่กันแล้ว
        dnsFiles: dnsFiles || null,
        dnsLogging: {
            sites: dnsSites,
