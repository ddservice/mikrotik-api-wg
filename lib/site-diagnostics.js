...
/**
 * สาขานี้ "เข้าถึงผ่านอุโมงค์ WireGuard" หรือไม่
 *
 * ตัดสินจาก host ที่ใช้ต่อจริง ไม่ใช่จากการมีฟิลด์ wireguardIp ค้างอยู่
 *
 * เดิมนับ wireguardIp ด้วย ทำให้ Auioun@WiFi ซึ่งต่อตรงผ่าน DDNS ถูกตรวจชั้น
 * WireGuard แล้วรายงาน fail ว่า "ไม่พบ peer ของ 10.10.88.1" ทั้งที่ 10.10.88.1
 * คือ IP ของ VPS เอง ไม่ใช่ peer และสาขานั้นก็ไม่ได้ใช้อุโมงค์เลย
 * (เจอตอนรันจริงบน VPS 2026-08-31 — ผลรวมขึ้น "ผ่านครบ" แต่มีชั้นหนึ่ง fail
 *  ซึ่งขัดกันเองและทำให้คนอ่านไม่เชื่อเครื่องมือ)
 */
function usesWireguard(config) {
    if (config.connectionType === 'direct') return false;
