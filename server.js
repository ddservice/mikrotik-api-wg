...
// Generate Multi-WAN RouterOS v7 CLI Script
app.post('/api/multiwan/generate-script', requireAuth(['admin', 'co-admin']), async (req, res) => {
    try {
        ...
        const script = `# ==============================================================================
# Enterprise RouterOS v7+ Multi-WAN Setup Script (Site Specific)
...
/ip route add dst-address=${dns1}/32 gateway=${wan1} scope=10 target-scope=10 comment="WAN1 Host Check"
/ip route add dst-address=0.0.0.0/0 gateway=${dns1} check-gateway=ping distance=1 scope=30 target-scope=11 comment="Default Primary WAN1"
/ip route add dst-address=0.0.0.0/0 gateway=${dns1} check-gateway=ping distance=1 routing-table=to_WAN1 scope=30 target-scope=11 comment="Table to_WAN1"

/ip route add dst-address=${dns2}/32 gateway=${wan2Gw} scope=10 target-scope=10 comment="WAN2 Host Check"
/ip route add dst-address=0.0.0.0/0 gateway=${dns2} check-gateway=ping distance=2 scope=30 target-scope=11 comment="Default Backup WAN2"
/ip route add dst-address=0.0.0.0/0 gateway=${dns2} check-gateway=ping distance=1 routing-table=to_WAN2 scope=30 target-scope=11 comment="Table to_WAN2"
...
${cfg.dnsHijack ? `add chain=dstnat protocol=udp dst-port=53 in-interface-list=!WAN action=redirect to-ports=53 comment="Force DNS Hijack UDP"
add chain=dstnat protocol=tcp dst-port=53 in-interface-list=!WAN action=redirect to-ports=53 comment="Force DNS Hijack TCP"` : ''}
${cfg.hairpinNat ? `add chain=srcnat src-address=192.168.0.0/16 dst-address=192.168.0.0/16 action=masquerade comment="Hairpin NAT"` : ''}

# 4. Mangle: FastTrack Bypass, Sticky HTTPS, PBR, Weighted PCC
/ip firewall mangle
${cfg.fasttrackBypass ? `add chain=prerouting action=accept connection-state=new comment="Bypass FastTrack"` : ''}
add chain=prerouting protocol=tcp dst-port=443 connection-state=new dst-address-type=!local in-interface-list=!WAN action=mark-connection new-connection-mark=HTTPS_STICKY passthrough=yes comment="Sticky 443"

# Policy-Based Routing (PBR)
add chain=prerouting src-address=${vlan10} dst-address-type=!local action=mark-routing new-routing-mark=to_WAN1 passthrough=no comment="PBR VLAN 10 -> WAN1"
add chain=prerouting src-address=${vlan20} dst-address-type=!local action=mark-routing new-routing-mark=to_WAN2 passthrough=no comment="PBR VLAN 20 -> WAN2"

# Weighted PCC Load Balancing (2:1 Ratio)
add chain=prerouting dst-address-type=local action=accept comment="Accept Local"
add chain=prerouting in-interface-list=!WAN connection-mark=no-mark dst-address-type=!local per-connection-classifier=both-addresses-and-ports:3/0 action=mark-connection new-connection-mark=WAN1_CONN passthrough=yes comment="PCC 3/0 -> WAN1"
add chain=prerouting in-interface-list=!WAN connection-mark=no-mark dst-address-type=!local per-connection-classifier=both-addresses-and-ports:3/1 action=mark-connection new-connection-mark=WAN1_CONN passthrough=yes comment="PCC 3/1 -> WAN1"
add chain=prerouting in-interface-list=!WAN connection-mark=no-mark dst-address-type=!local per-connection-classifier=both-addresses-and-ports:3/2 action=mark-connection new-connection-mark=WAN2_CONN passthrough=yes comment="PCC 3/2 -> WAN2"

add chain=prerouting connection-mark=WAN1_CONN in-interface-list=!WAN action=mark-routing new-routing-mark=to_WAN1 passthrough=no comment="Routing WAN1"
add chain=prerouting connection-mark=WAN2_CONN in-interface-list=!WAN action=mark-routing new-routing-mark=to_WAN2 passthrough=no comment="Routing WAN2"
add chain=prerouting connection-mark=HTTPS_STICKY in-interface-list=!WAN action=mark-routing new-routing-mark=to_WAN1 passthrough=no comment="Routing Sticky HTTPS"
${cfg.mssClamping ? `add chain=forward protocol=tcp tcp-flags=syn action=change-mss new-mss=clamp-to-pmtu comment="MSS Clamping"` : ''}
...
/tool netwatch
add host=${dns1} type=icmp interval=5s timeout=1000ms comment="WAN1 Netwatch" up-script="/tool fetch url=\\"https://api.telegram.org/bot${tgToken}/sendMessage?chat_id=${tgChatId}&text=%E2%9C%85+WAN1+ONLINE\\" keep-result=no" down-script="/tool fetch url=\\"https://api.telegram.org/bot${tgToken}/sendMessage?chat_id=${tgChatId}&text=%F0%9F%9A%A8+WAN1+OFFLINE\\" keep-result=no"
add host=${dns2} type=icmp interval=5s timeout=1000ms comment="WAN2 Netwatch" up-script="/tool fetch url=\\"https://api.telegram.org/bot${tgToken}/sendMessage?chat_id=${tgChatId}&text=%E2%9C%85+WAN2+ONLINE\\" keep-result=no" down-script="/tool fetch url=\\"https://api.telegram.org/bot${tgToken}/sendMessage?chat_id=${tgChatId}&text=%F0%9F%9A%A8+WAN2+OFFLINE\\" keep-result=no"
`;
        res.json({ script });
    } catch (e) {
        res.status(500).json({ error: e.message });
